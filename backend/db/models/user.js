...
// The API login route will be hit with a request body holding a valid credential (either username or email) and
...
// If there is a match, the API login route should send back a JWT in an HTTP-only cookie and a response body.
...
// The API signup handler will create a User with the username, an email, and a hashedPassword created
//  from the input password.
// If the creation is successful, the API signup route should send back a JWT in an HTTP-only cookie
// and a response body. The JWT and the body will hold the user's id, username, and email.

// //The backend logout flow will be based on the following plan:
// The API logout route will be hit with a request.
// The API logout handler will remove the JWT cookie set by the login or signup
